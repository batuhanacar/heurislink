...
export function setup() {
    const payload = JSON.stringify({
        originalUrl: `https://example.com/kafka-load-${Date.now()}`,
    });
    ...
    const success = check(response, {
        'setup URL created': (res) => res.status === 201,
    });
    ...
    console.log(`Kafka load shortCode: ${shortCode}`);
...
export default function (data) {
    const response = http.get(
        `${BASE_URL}/${data.shortCode}`,
        {
            redirects: 0,
            tags: {
                endpoint: 'click-analytics',
            },
        }
    );

    check(response, {
        'redirect succeeds': (res) =>
            res.status === 301 ||
            res.status === 302 ||
            res.status === 307 ||
            res.status === 308,
